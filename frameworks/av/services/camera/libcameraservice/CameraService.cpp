...
sp<ICameraServiceProxy> CameraService::getOtherCameraServiceProxy() {
    sp<IServiceManager> sm = otherdefaultServiceManager();
    if(sm == nullptr)
        return nullptr;
...
void CameraService::pingCameraServiceProxy() {
    ...
    sp<ICameraServiceProxy> otherproxyBinder = getOtherCameraServiceProxy();
    if (otherproxyBinder == nullptr) return;
    otherproxyBinder->pingForUserUpdate();
}
...
void CameraService::updateProxyDeviceState(ICameraServiceProxy::CameraState newState,
    ...
    sp<ICameraServiceProxy> otherproxyBinder = getOtherCameraServiceProxy();
    if (otherproxyBinder == nullptr) return;
    otherproxyBinder->notifyCameraState(id, newState);
}
