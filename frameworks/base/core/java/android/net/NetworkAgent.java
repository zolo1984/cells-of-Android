...
import android.net.cells.ParcelableBoolean;
import android.net.cells.ParcelableArrayUidRange;
import android.net.cells.ParcelableInt;
...
public abstract class NetworkAgent extends Handler {
    ...
    public NetworkAgent(Looper looper, Context context, String logTag, NetworkInfo ni,
        ...
        if(bVM){
            ...
            if(b != null){
                ConnectivityManager cm =new ConnectivityManager(mContext,IConnectivityManager.Stub.asInterface(b));
                netId = cm.registerNetworkAgent(new Messenger(this), new NetworkInfo(ni),
                        new LinkProperties(lp), new NetworkCapabilities(nc), score, null);
            }else{
                netId = -1;
            }
        }else{
        ...
        log("Registering NetworkAgent netId="+netId);
    }
                ...
                if (mAsyncChannel != null) {
                    ...
                    synchronized (mPreConnectedQueue) {
                        ...
                        for (Message m : mPreConnectedQueue) {
                            log("NetworkAgent sendMessage:"+m.what);
                            ac.sendMessage(m);
                        }
    ...
    public void sendNetworkScore(int score) {
        ...
        queueOrSendMessage(EVENT_NETWORK_SCORE_CHANGED, new ParcelableInt(score));
    }
    ...
    public void addUidRanges(UidRange[] ranges) {
        queueOrSendMessage(EVENT_UID_RANGES_ADDED, new ParcelableArrayUidRange(ranges));
    }
    ...
    public void removeUidRanges(UidRange[] ranges) {
        queueOrSendMessage(EVENT_UID_RANGES_REMOVED, new ParcelableArrayUidRange(ranges));
    }
    ...
    public void explicitlySelected(boolean acceptUnvalidated) {
        queueOrSendMessage(EVENT_SET_EXPLICITLY_SELECTED, new ParcelableBoolean(acceptUnvalidated));
    }
