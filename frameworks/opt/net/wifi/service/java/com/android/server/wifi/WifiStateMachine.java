...
//                        startScanNative(WifiNative.SCAN_WITHOUT_CONNECTION_SETUP, null);
    ...
    private class WifiNetworkAgent extends NetworkAgent {
        public WifiNetworkAgent(Looper looper, Context context, String logTag, NetworkInfo ni,
            NetworkCapabilities nc, LinkProperties lp, int score,boolean bVM) {
            super(looper, context, logTag, ni, nc, lp, score, bVM); 
        }
    ...
    private class cellsWifiNetworkAgent {
        ...
        public WifiNetworkAgent mVMSystemAgent;

        public final int netId;

        private static final String VM_NEAR_IFACE_ADDR      = "172.17.3.3";
        ...
        public cellsWifiNetworkAgent(Looper l, Context c, String TAG, NetworkInfo ni,
            ...
            mInitSystemAgent = new WifiNetworkAgent(l,c,TAG,ni,nc,lp,score,false);
            netId = mInitSystemAgent.netId;

            IBinder b = ServiceManager.getOtherSystemService(Context.CONNECTIVITY_SERVICE);
            if(b != null){

                LinkProperties vmlp = null;
                if(lp != null){
                    vmlp = new LinkProperties(lp);
                    ...
                    List<LinkProperties> stacks = lp.getStackedLinks();
                    for (LinkProperties linkP : stacks) {
                        vmlp.removeStackedLink(linkP.getInterfaceName());
                    }

                    {
                        InetAddress gateway = NetworkUtils.numericToInetAddress("172.17.3.2");
                        InetAddress dest    = NetworkUtils.numericToInetAddress("0.0.0.0");
                        RouteInfo route = new RouteInfo(new LinkAddress(dest, 0),gateway,lp.getInterfaceName());
                        vmlp.addRoute(route);
                    }

                    {
                        InetAddress gateway = NetworkUtils.numericToInetAddress("0.0.0.0");
                        InetAddress dest    = NetworkUtils.numericToInetAddress("172.17.0.0");
                        RouteInfo route = new RouteInfo(new LinkAddress(dest, 16),gateway,lp.getInterfaceName());
                        vmlp.addRoute(route);
                    }

                    InetAddress addr = NetworkUtils.numericToInetAddress(VM_NEAR_IFACE_ADDR);
                    vmlp.addLinkAddress(new LinkAddress(addr, VM_PREFIX_LENGTH));
                }

                mVMSystemAgent = new WifiNetworkAgent(l,c,TAG,ni,nc,vmlp,score,true);
            }
        }
        ...
        */
        public void sendLinkProperties(LinkProperties linkProperties) {
            ...
            if(mVMSystemAgent != null){

                LinkProperties vmlp = null;
                if(linkProperties != null){
                    vmlp = new LinkProperties(linkProperties);
                    List<LinkAddress> addrs = linkProperties.getLinkAddresses();
                    for (LinkAddress linkAddress : addrs) {
                        vmlp.removeLinkAddress(linkAddress);
                    }
                    ...
                    List<LinkProperties> stacks = linkProperties.getStackedLinks();
                    for (LinkProperties linkP : stacks) {
                        vmlp.removeStackedLink(linkP.getInterfaceName());
                    }

                    {
                        InetAddress gateway = NetworkUtils.numericToInetAddress("172.17.3.2");
                        InetAddress dest    = NetworkUtils.numericToInetAddress("0.0.0.0");
                        RouteInfo route = new RouteInfo(new LinkAddress(dest, 0),gateway,linkProperties.getInterfaceName());
                        vmlp.addRoute(route);
                    }

                    {
                        InetAddress gateway = NetworkUtils.numericToInetAddress("0.0.0.0");
                        InetAddress dest    = NetworkUtils.numericToInetAddress("172.17.0.0");
                        RouteInfo route = new RouteInfo(new LinkAddress(dest, 16),gateway,linkProperties.getInterfaceName());
                        vmlp.addRoute(route);
                    }
                ...
                mVMSystemAgent.sendLinkProperties(vmlp);
            }
        }
        ...
        */
        public void sendNetworkInfo(NetworkInfo networkInfo) {
            ...
            if(mVMSystemAgent != null)
                mVMSystemAgent.sendNetworkInfo(networkInfo);
        }
        ...
        */
        public void sendNetworkCapabilities(NetworkCapabilities networkCapabilities) {
            ...
            if(mVMSystemAgent != null)
                mVMSystemAgent.sendNetworkCapabilities(networkCapabilities);
        }
        ...
        */
        public void sendNetworkScore(int score) {
            ...
            if(mVMSystemAgent != null)
                mVMSystemAgent.sendNetworkScore(score);
        }
        ...
        */
        public void explicitlySelected(boolean acceptUnvalidated) {
            ...
            if(mVMSystemAgent != null)
                mVMSystemAgent.explicitlySelected(acceptUnvalidated);
        }
        ...
        */
        public void onPacketKeepaliveEvent(int slot, int reason) {
            ...
            if(mVMSystemAgent != null)
                mVMSystemAgent.onPacketKeepaliveEvent(slot,reason);
        }
    }
