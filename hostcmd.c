...
{
	int _e = HOST_BASE_CMD<<16||SYSTEM_SWITCH;
	...
	send_msg(1,sizeof(int),(const char *)&_e,NULL,NULL);

	system("fission switch cell1");
...
static void init_net_work()
{
	...
	ALOGD("brctl addbr br0 errno=%s",strerror(errno));
	system("ip addr add 172.16.1.254/16 dev br0");
	ALOGD("ip addr add 172.16.1.254/16 dev br0 errno=%s",strerror(errno));
	system("ip link set dev br0 up");
	ALOGD("ip link set dev br0 up errno=%s",strerror(errno));
	sleep(2);
	system("brctl addif br0 cell1_wlan0");
	ALOGD("brctl addif br0 cell1_wlan0 errno=%s",strerror(errno));
	system("ip addr add 172.16.1.5/16 dev cell1_wlan0");
	ALOGD("ip addr add 172.16.1.5/16 dev cell1_wlan0 errno=%s",strerror(errno));
	system("ip link set dev cell1_wlan0 up");
	ALOGD("ip link set dev cell1_wlan0 up errno=%s",strerror(errno));
};
