 ...
 */
 ...
 #ifdef CONFIG_DRV_NS
 #define DEBUG
 #define DRV_NS_EVDEV_DEBUG
 
 #ifdef DRV_NS_EVDEV_DEBUG
 #define pr_fmt(fmt) \
	 "[%d] drvns:evdev [%s:%d]: " fmt, \
	 current->pid, __func__, __LINE__
 #else
 #define pr_fmt(fmt) \
	 "[%d] drvns:evdev: " fmt, current->pid
 #endif
 #endif /* CONFIG_DRV_NS */
 ...
 {
	 return (is_active_evdev_drv_ns(client->evdev_ns)) || client->is_always_active;
 }
	 ...
	 case DRV_NS_EVENT_DEACTIVATE:
		 ...
		 list_for_each_entry(client, &evdev_ns->clients, list)
		 {
			 ...
			 list_for_each_entry_safe(kd, n,
				 ...
				 event.code = kd->code;
				 event.time = ktime_to_timeval(client->clkid == CLOCK_MONOTONIC ?time_mono : time_real);
