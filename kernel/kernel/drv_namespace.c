 ...
 *
 * Copyright (c) 2016-2021 cells
 * Authors:
 *    guoweibin <gwb@cells.com>
 *
 * Driver namespaces:
 *
...
extern void container_add(struct drv_namespace *drv_ns);
extern void container_switch(struct drv_namespace *drv_ns);

struct pid_namespace *sys_mode_to_pid_ns[CELLS_SYSTEM_NUM];
...
int get_drv_ns_idx(void)
{
	static struct drv_namespace *drv_ns_idx[MAX_CONTAINER];
	static unsigned char init_flag;
	struct drv_namespace *cur_ns = current_drv_ns();
	const char *cells_tag_list[CELLS_SYSTEM_NUM] = {"cell1", "cell2"};
	int i;

	if (0 == init_flag) {
		memset(drv_ns_idx, 0, sizeof(drv_ns_idx));
		init_flag = 1;
	}

	if (&init_drv_ns == cur_ns)
		return INIT_DRV_NS_IDX;

	for (i = 0; i < CELLS_SYSTEM_NUM; i++) {
		if (0 == strcmp(cur_ns->tag, cells_tag_list[i]))
			return i+1;
	}

	for (i = 0; i < MAX_CONTAINER; i++) {
		if (NULL == drv_ns_idx[i]) {
			drv_ns_idx[i] = cur_ns;
			return i + CELLS_SYSTEM_NUM + 1;
		}

		if (cur_ns == drv_ns_idx[i])
			return i;
	}

	pr_err("[get_drv_ns_idx] too much container created!");

	return -1;
}

void get_drv_ns_tag(char *to, struct drv_namespace *drv_ns)
{
	/* buf must be at least sizeof(drv_ns->tag) in size */
	to[DRV_NS_TAG_LEN] = '\0';
	strncpy(to, drv_ns->tag, DRV_NS_TAG_LEN);
}

void set_drv_ns_tag(struct drv_namespace *drv_ns, char *from)
{
	drv_ns_lock(drv_ns);
	strncpy(drv_ns->tag, from, DRV_NS_TAG_LEN);
	pr_err("[set_drv_ns_tag] init drv ns tag : %s \n",drv_ns->tag);
	if (!strncmp(from, SECURITY_SYSTEM_TAG, DRV_NS_TAG_LEN))
		sys_mode_to_pid_ns[SECURITY_SYSTEM_MODE] = drv_ns->pid_ns;
	else if (!strncmp(from, GENERAL_SYSTEM_TAG, DRV_NS_TAG_LEN))
		sys_mode_to_pid_ns[GENERAL_SYSTEM_MODE] = drv_ns->pid_ns;
	else
		pr_err("[set_drv_ns_tag] invalited drv ns tag\n");
	drv_ns_unlock(drv_ns);
}
...
static ssize_t drv_ns_proc_write(struct file *file,
				 ...
				 size_t count, loff_t *ppos, int setactive)
{
	...
	drv_ns = get_drv_ns_by_vpid(new_pid);
	if (!drv_ns) {
	...
	if (setactive) {
		...
		/* set drv_ns tag if format was <pid>:<tag> */
		/* (safe: last byte of tag always remains NULL) */
		set_drv_ns_tag(drv_ns, tag_str);
	} else {
		pr_warn("drv_ns: bad PID:tag format '%s'\n", pid_str);
		count = -EINVAL;
	}
...
{
	return drv_ns_proc_write(file, buffer, count, ppos, 1);
}
...
static ssize_t proc_ns_tag_write(struct file *file,
				 ...
				 size_t count, loff_t *ppos)
{
	return drv_ns_proc_write(file, buffer, count, ppos, 0);
}
...
static __init int drv_namespace_init(void)
{
	...
	pr_err("[drv_namespace_init] success exit...\n");
