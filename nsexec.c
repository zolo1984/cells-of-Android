...
#endif
int fission_init_interface(struct cell_args *cell_args, int intf_idx)
{
	int sysIdx;
	char *veth_tmp_name = "veth_tmp";
	char cmd_buf[256];
	const char *veth_host_name = NULL;
	const char *veth_name = NULL;

	if (0 == strcmp(cell_args->cellname, "cell1"))
		sysIdx = 1; /* security system keep the same value with container driver */
	else
		sysIdx = 0; /* generay system keep the same value with container driver */

	veth_host_name = getInterfaceHostName(intf_idx, sysIdx);
	veth_name = getInterfaceName(intf_idx);

	veth_peer_create(veth_tmp_name, veth_host_name);
	netdev_move_to_netns(veth_tmp_name, cell_args->init_pid, veth_name);

	return 0;
}
int fission_init_net(struct cell_args *cell_args)
{
	int i;

	for (i = 0; i < FISSION_INTERFACE_NUM; i++)
		fission_init_interface(cell_args, i);
	return 0;
}
/* guoweibin add veth end*/
...
static int do_child(void *vargv)
{
	...
	ALOGD("wifiproxy: %d", start_args->wifiproxy);
	...
	ALOGD("wifi_socket: %d", cell_args->wifi_socket);
	...
	if (cell_args->wifi_socket >= 0)
		close(cell_args->wifi_socket);
...
static int do_clone(struct cell_args *cell_args)
{
	...
	char buf[20];
	char veth_name[256];
	char *fission_host_ip[2] = {FISSION1_HOST_IP, FISSION2_HOST_IP};
	int fission_idx = 0;
	char *syserr;
	int e;
...
void tear_down_cell(struct cell_args *cell_args, struct pty_info *console_pty)
{
	...
	if (args->wifiproxy)
		destroy_wifi_proxy_socket(cell_args->rootdir);
	/*
	if (args->RIL_PROXY) {
		...
		char *name, struct pty_info *console_pty)
{
	...
	ALOGD("args->wifiproxy = %d\n", args->wifiproxy);
	cell_args->wifi_socket = -1;
	if (args->wifiproxy) {
		ALOGD("create wifi proxy socket....\n");
		ret = create_wifi_proxy_socket(rootdir);
		if (ret < 0) {
			ALOGE("failed to create '%s' wifi_proxy_socket "
			     "(WiFi probably won't work)", name);
		}
		cell_args->wifi_socket = ret;
	}
	...
	/* Tell wifi proxy socket which init-pid is using which socket */
	if (args->wifiproxy)
		activate_wifi_proxy_socket(rootdir, pid);
