...
#define LOG_TAG "fingerprintd-jiang"
...
static int get_active_system(void)
{
...
FingerprintDaemonProxy::FingerprintDaemonProxy() : 
    mModule(NULL), mDevice(NULL), mCallback(NULL),mCallback2(NULL),mInitCount(0) {
...
void FingerprintDaemonProxy::hal_notify_callback(const fingerprint_msg_t *msg) {
    ...
    const sp<IFingerprintDaemonCallback> callback2 = instance->mCallback2;
    const int active_sysid = get_active_system();

    if (callback == NULL && active_sysid == HOST) {
        ALOGE("Invalid callback object");
        return;
    }
    if (callback2 == NULL && active_sysid == VM){
        ALOGE("Invalid callback2 object");
        return;
    }
    ...
    switch (msg->type) {
        case FINGERPRINT_ERROR:
            ...
            if(active_sysid != VM)
                ...
                callback2->onError(device, msg->data.error);
            break;
        case FINGERPRINT_ACQUIRED:
            ...
            if(active_sysid != VM)
                ...
                callback2->onAcquired(device, msg->data.acquired.acquired_info);
            break;
        case FINGERPRINT_AUTHENTICATED:
            ...
            if(active_sysid != VM)
                ...
                callback2->onAuthenticated(device,
                        msg->data.authenticated.finger.fid,
                        msg->data.authenticated.finger.gid);                
        ...
        case FINGERPRINT_TEMPLATE_ENROLLING:
            ...
            if(active_sysid != VM)
                ...
                callback2->onEnrollResult(device,
                        msg->data.enroll.finger.fid,
                        msg->data.enroll.finger.gid,
                        msg->data.enroll.samples_remaining);            
            break;
...
void FingerprintDaemonProxy::init(const sp<IFingerprintDaemonCallback>& callback) {
    ALOG(LOG_ERROR, LOG_TAG, "init(sysid=%d)\n", 0);
...
void FingerprintDaemonProxy::init2(const sp<IFingerprintDaemonCallback>& callback,int sysid) {
    ALOG(LOG_ERROR, LOG_TAG, "init2(sysid=%d)\n", sysid);
    if(sysid != VM){
        if (mCallback != NULL && IInterface::asBinder(callback) != IInterface::asBinder(mCallback)) {
            IInterface::asBinder(mCallback)->unlinkToDeath(this);
        }
        ...
        if (mCallback2 != NULL && IInterface::asBinder(callback) != IInterface::asBinder(mCallback2)) {
            IInterface::asBinder(mCallback2)->unlinkToDeath(this);
        }
        IInterface::asBinder(callback)->linkToDeath(this);
        mCallback2 = callback;
    }
...
int32_t FingerprintDaemonProxy::closeHal() {
    ...
    if(android_atomic_dec(&mInitCount) > 0){
        return 0;
    }
...
void FingerprintDaemonProxy::binderDied(const wp<IBinder>& who) {
    ...
    if (IInterface::asBinder(mCallback) == who) {
        mCallback = NULL;
    }else if (IInterface::asBinder(mCallback2) == who) {
        mCallback2 = NULL;
    }
}
